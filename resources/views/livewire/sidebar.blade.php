...
{{--        ///--}}
    ...
    {{-- Expanded Sidebar--}}
    <div class="flex-1 flex flex-col min-h-0  rounded-r-lg  w-48 darklight" x-show="sidebar" x-cloak>
        <div class="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
            <div class=" text-center">
            ...
            <nav class="mt-5 flex-1 px-2  space-y-1" aria-label="Sidebar">
                @if(isset($sidebarlinks) && $sidebarlinks)
                    @foreach($sidebarlinks as $links)
                        <a href="@if(\Illuminate\Support\Facades\Route::has($links['route'])) {{route($links['route'])}} @else # @endif" data-bs-toggle="tooltip" data-bs-placement="right" title="Visit {{\Illuminate\Support\Str::ucfirst($links['name'])}}"
                           class=" group flex items-center px-2 py-2 text-sm font-medium rounded-md font-medium" x-state:on="Current" x-state:off="Default" x-state-description="Current: &quot;bg-gray-900 text-white&quot;, Default: &quot;text-gray-300 hover:bg-gray-700 hover:text-white&quot;">
    ...
    <div x-show="leftMSidebar" class="md:hidden " x-description="Off-canvas menu for mobile, show/hide based on off-canvas menu state." x-ref="dialog" aria-modal="true" style="display: none;">
        <div class="fixed inset-0 z-40 flex">
            ...
            <div x-show="leftMSidebar" class="fixed inset-0  bg-opacity-75" @click="leftMSidebar = false"
                 x-transition:enter="transition-opacity ease-linear duration-300" x-transition:enter-start="opacity-0"
                 x-transition:enter-end="opacity-100" x-transition:leave="transition-opacity ease-linear duration-300"
                 x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0"
                 x-description="Off-canvas menu overlay, show/hide based on off-canvas menu state."  aria-hidden="true" style="display: none;">
            </div>

            <div x-show="leftMSidebar" class="relative max-w-xs w-full pt-5 pb-4 flex-1 flex flex-col bg-gray-300 text-gray-600"
                 x-transition:enter="transition ease-in-out duration-300 transform" x-transition:enter-start="-translate-x-full"
                 x-transition:enter-end="translate-x-0" x-transition:leave="transition ease-in-out duration-300 transform"
                 x-transition:leave-start="translate-x-0" x-transition:leave-end="-translate-x-full"
                 x-description="Off-canvas menu, show/hide based on off-canvas menu state."  style="display: none;">

                <div x-show="leftMSidebar" x-transition:enter="ease-in-out duration-300" x-transition:enter-start="opacity-0"
                     x-transition:enter-end="opacity-100" x-transition:leave="ease-in-out duration-300"
                     x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="absolute top-1 right-0 -mr-14 p-1"
                     x-description="Close button, show/hide based on off-canvas menu state." style="display: none;">
                    ...
                    <nav class="h-full flex flex-col">
                        <div class="space-y-1">
                            @if(isset($sidebarlinks) && $sidebarlinks)
                                @foreach($sidebarlinks as $links)
                                    <a href="@if(\Illuminate\Support\Facades\Route::has($links['route'])) {{route($links['route'])}} @else # @endif" data-bs-toggle="tooltip" data-bs-placement="right" title="Visit {{\Illuminate\Support\Str::ucfirst($links['name'])}}"
                                       class="py-2 px-3 rounded-md flex items-center text-sm font-medium " x-state:on="Current" x-state:off="Default" x-state-description="Current: &quot;bg-indigo-800 text-white&quot;, Default: &quot;text-indigo-100 hover:bg-indigo-800 hover:text-white&quot;">
